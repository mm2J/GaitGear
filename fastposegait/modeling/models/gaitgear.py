...
from ..components import GearBlock, GearBasicBlock, SemanticPyramidPooling, PackSequenceWrapper, SeparateFCs, \
    SeparateBNNecks
from ..graph import Graph
import numpy as np


class GaitGear(BaseModel):
...
class GagStream(nn.Module):
    def __init__(self, in_c, first_c, share_num, is_Embed, graph, joint_format='coco'):
        ...
        main_stream = []
        # stage1
        main_stream.append(GearBasicBlock(in_channel=first_c, out_channel=in_c[1], A=self.A, is_Embed=is_Embed, num_point=num_point, joint_format=joint_format))
        # stage2
        for i in range(1, share_num):
            main_stream.append(GearBlock(in_channel=in_c[i], out_channel=in_c[i + 1], A=self.A, is_Embed=is_Embed, num_point=num_point, joint_format=joint_format))
        ...
        for i in range(share_num, len(in_c) - 1):
            parts2_1.append(GearBlock(in_channel=in_c[i], out_channel=in_c[i + 1], A=self.A, is_Embed=is_Embed, num_point=num_point, joint_format=joint_format))
        self.parts2_1 = nn.ModuleList(parts2_1)

        ### parts2_2
        parts2_2 = []
        for i in range(share_num, len(in_c) - 1):
            parts2_2.append(GearBlock(in_channel=in_c[i], out_channel=in_c[i + 1], A=self.A, is_Embed=is_Embed, num_point=num_point, joint_format=joint_format))
